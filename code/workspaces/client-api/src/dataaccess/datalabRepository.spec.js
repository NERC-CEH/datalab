...
describe('datalabRepository', () => {
  ...
  it('getAll returns expected snapshot', () => datalabRepository.getAll('user').then((datalabs) => {
    expect(mockDatabase().query()).toEqual({});
    expect(datalabs).toMatchSnapshot();
  }));

  it('getByName returns expected snapshot', () => datalabRepository.getByName(undefined, 'datalab').then((datalabs) => {
