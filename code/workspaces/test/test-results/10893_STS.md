...
>>> | 6 | Confirm the restart | The page is reloaded and there is a message displayed that iforms the user that the restart was successful | PASS |
...
>>> | 3 | View the response for the second to last api call | Contains the projects the user has access to and the admin status:  "system:catalogue:admin". It also contains the catalogue status e.g. catalogue publisher | PASS |
...
>>> | 7 | Select some filters e.g. instance admin and project admin | The page is filtered to only show users that meet these filter requirements. Checkboxes are clearly marked as selected | PASS |
