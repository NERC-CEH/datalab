...
| 4 | Click on the sign up button | User is re-directed to <https://testlab.test-datalabs.nerc.ac.uk/sign-upand> an email is created. This email is addressed to appsupport@ceh.ac.uk |  |
...
| **JIRA ID(s):** | [NERCDL-910](https://jira.ceh.ac.uk/browse/NERCDL-908?jql=key%20in%20(NERCDL-910)) |OVERALL RESULT: | PASS |
| ------------ | --------- | --------- | ------|
| **Number** | **Step** | **Expected Result** | **Result** |
| 1 | Ensure that the logged in user has the data manager role | User is a data manager and can see "Asset Repo" in the header/navigation bar at the top of the page| PASS |
| 2 | Click on "Asset Repo" | User is taken to "/assets/add-metadata". There is a blank form with the following fields: name, version, file location, master URL, owners, visibility. Asset Id is listed as "Awaiting Submission" | PASS |
---
...
| 8 | Set the master URL to any valid URL e.g. <https://google.com>| URL is set. There are no validation errors. | PASS |
| 9 | *Removed*
...
| 20 | Click on confirm | A confirmation in the UI is shown informing the user that the metadata is set. Asset id now has an actual id string. All fields are now read only. There is a button labelled "Add another" at the bottom | PASS |
---
...
| 2 | Attempt to submit an exact copy of a form that has already been submitted | Form is not submitted because the name & version are repeated, the file location is repeated and master URL and version are repeated | PASS |
...
| 5 | Attempt to submit a form where the master URL and version are the only fields that match the submitted form | Form is not submitted due to the master name and version matching a submitted form | PASS |
---
...
| **JIRA ID(s):** | [NERCDL-936](https://jira.ceh.ac.uk/browse/NERCDL-936),  |OVERALL RESULT: |  |
| ------------ | --------- | --------- | ------|
| **Number** | **Step** | **Expected Result** | **Result** |
