...
>>> | 3 | Click on the edit button | A dialog is opened containing the display name and description. There are three buttons: Apply, Clear Changes and Cancel. All buttons except "Cancel" are disabled.  | PASS |
...
>>> | 6 | Press "Clear Changes" | The original text is now present in the text fields | PASS |
>>> | 7 | Replace any text with a string that contains text, numbers and special characters | Text is replaced | PASS |
>>> | 8 | Press "Apply" | Changes are applied and there is a confirmation message displayed in the UI | PASS |
...
>>> | 4 | Fill in the fields as required and set the data store mount to the pre-existing NFS store. Set the notebook type to Jupyterlab  | NFS store and notebook type is set | PASS |
...
>>> | 7 | Repeat the above tests for pre-existing Gluster store | The same results as the NFS store (above) are seen | PASS |

---

>>>###### **TC2.2.3.3 User can restart stacks in the UI**
>>> | **JIRA ID(s):** | [NERCDL-648](https://jira.ceh.ac.uk/browse/NERCDL-648?jql=key%20in%20) |OVERALL RESULT: | PASS |
>>> | ------------ | --------- | --------- | ------|
>>> | **Number** | **Step** | **Expected Result** | **Result** |
>>> | 1 | View the notebooks section of Datalabs | Existing notebooks are present | PASS |
>>> | 2 | Click on the three dot icon next to the notebook | User has the options to edit, share, restart or delete | PASS |
>>> | 3 | Click on restart | A dialog is shown asking the user to confirm or cancel the restart | PASS |
>>> | 4 | Press cancel | The dialog is closed | PASS |
>>> | 5 | Click on restart | The dialog is shown again | PASS |
>>> | 6 | Confirm the restart | The page is reloaded and there is a message displayed that informs the user that the restart was successful | PASS |
>>> | 7 | Click on the drop down and select the edit option | A dialog window opens with the existing details | PASS |
>>> | 8 | Edit the details of the notebook and press save | The notebook is edited | PASS |
>>> | 9 | Repeat all of the above tests as a non-admin user | Same results are observed as an admin user | PASS |
---
...
>>> | 2 | Click on the create button | A dialog is shown with the following fields: Display Name, Notebook Type, URL, Data Store, Source Path, Description and Sharing Status | PASS |
>>> | 3 | Fill out the required fields. Set the type to R studio | And extra field appears when R Studio is selected. This is a version field. 4.0.1 is the default value | PASS |
>>> | 4 | Click on the version field | A dropdown with two options is displayed: 4.0.1 & 3.5.3 | PASS |
...
>>> | **JIRA ID(s):** | [NERCDL-919](https://jira.ceh.ac.uk/browse/NERCDL-919), [NERCDL-934](https://jira.ceh.ac.uk/browse/NERCDL-934) |OVERALL RESULT: | PASS |
>>> | ------------ | --------- | --------- | ------|
>>> | **Number** | **Step** | **Expected Result** | **Result** |
>>> | 1 | Ensure there are available assets for testing through the asset list page. Can be added through add asset page if required. | There is at least one of: a public asset with fileLocation, an asset available only to the project that is being used for testing with a fileLocation, an asset that is not available to the project used for testing with a fileLocation, and a project that does not have a fileLocation. | PASS |
>>> | 2 | Navigate to Notebooks page of pre-existing project (create project if necessary). | Pre-existing notebooks are listed here. | PASS |
>>> | 3 | Ensure logged in to account with permission to create notebooks. | Create Notebook button is displayed at the bottom of the list of notebooks | PASS |
>>> | 4 | Click on the Create Notebook button | A dialog is shown with a form to fill in. One of the fields is called assets. | PASS |
>>> | 5 | Click on assets multiselect field | List of assets available to project is shown. This should only include public assets with a fileLocation and assets specifically available to project used for testing with fileLocation. | PASS |
>>> | 6 | Leaving all fields blank, click on create button. | Required fields show errors that they cannot be blank. The assets field shows no error (as can be left blank). | PASS |
>>> | 7 | Fill in required fields leaving assets blank. Click on create button. | Notebook is created. | PASS |
>>> | 8 | Click create notebook button. Fill in form with with details for new notebook including assets (which should actually exist on the repo file store). Click create button. | Notebook is created. | PASS |
>>> | 9 | Wait for the notebook with assets to become ready, and open it. | Notebook is opened. | PASS |
>>> | 10 | Open a Terminal | Terminal is opened. | PASS |
>>> | 11 | Enter `cd /assets/` and then `cd` into your selected real asset.  Perform an `ls`. | The asset contents are listed. | PASS |
>>> | 12 | Check you can read the data, e.g. `head readme.html`. | The asset data can be read. | PASS |
>>> | 13 | Attempt to write to the folder, e.g. `touch test.txt`. | You should receive an error message `Read-only file system`. | PASS |
---
...
>>> | 2 | Navigate to Notebooks page of pre-existing project (create project if necessary). | Pre-existing notebooks are listed here. | PASS |
>>> | 3 | Ensure logged in to account with permission to create notebooks. | Create Notebook button is displayed at the bottom of the list of notebooks | PASS |
>>> | 4 | Ensure there is a notebook that is available to edit (create notebook if necessary). | There is at least one notebook in the list of notebooks that can be edited for testing. | PASS |
>>> | 5 | Click on the notebook's three dot button and then on the edit option. | A dialog is shown with fields that can be edited. One of them is assets. The assets field is pre-populated with any assets the notebooks is using (note this may be dummy data during development). | PASS |
>>> | 6 | Click on assets multiselect field | List of assets available to project is shown. This should only include public assets with a fileLocation and assets specifically available to project used for testing with fileLocation. | PASS |
>>> | 7 | Using the asset multiselect it should be possible to add and remove assets from the notebook. | The list of assets associated with the notebook has changed to be those selected. | PASS |
>>> | 8 | Select assets which actually exist on the repo store and click apply. | The notebook should update successfully. | PASS |
>>> | 9 | Open the notebook. | Notebook is opened. | PASS |
>>> | 10 | Open a Terminal | Terminal is opened. | PASS |
>>> | 11 | Enter `cd /assets/` and then `cd` into your selected real asset.  Perform an `ls`. | The asset contents are listed. | PASS |
>>> | 12 | Check you can read the data, e.g. `head readme.html`. | The asset data can be read. | PASS |
>>> | 13 | Attempt to write to the folder, e.g. `touch test.txt`. | You should receive an error message `Read-only file system`. | PASS |
...
>>> | **JIRA ID(s):** | [NERCDL-841](https://jira.ceh.ac.uk/browse/NERCDL-841) |OVERALL RESULT: | PASS |
>>> | ------------ | --------- | --------- | ------|
>>> | **Number** | **Step** | **Expected Result** | **Result** |
>>> | 1 | Navigate to the notebooks page of a project in which the logged in account has permission to create a notebook. | See list of any currently running notebooks in the project with a Create Notebook button below it. | PASS |
>>> | 2 | Click the Create Notebook button. | Shown the create notebook dialog with fields for Type and - if the selected type allows different versions - Version. | PASS |
>>> | 3 | Note the types and versions that are available. | N/A | PASS |
>>> | 4 | Take a copy of the original image config map then update it to change the available types and the available versions of a notebook that allows different versions and restart the web-app pod (this may require a developer). Reload the page and click the Create Notebook button. | Shown the create notebook dialog where the values match those just set in the image config map. | PASS |
>>> | 5 | Reset the value of the config map to the original value and then restart the web-app pod (this may require a developer). | System is back to the same state as before the test was run. | PASS |
---
...
>>> | **Number** | **Step** | **Expected Result** | **Result** |
>>> | 1 | View the notebooks section of Datalabs | Existing notebooks are present | PASS |
>>> | 2 | Click on edit for one of the notebooks | A dialog is opened containing the display name, description and users with access. There are three buttons: Apply, Clear Changes and Cancel. All buttons except "Cancel" are disabled. | PASS |
>>> | 3 | Click on the "User's with access" field | A list of all user with access to Datalabs is shown | PASS |
...
>>> | **JIRA ID(s):** | [NERCDL-665](https://jira.ceh.ac.uk/browse/NERCDL-665?jql=key%20in%20(NERCDL-665)) |OVERALL RESULT: | PASS |
>>> | ------------ | --------- | --------- | ------|
>>> | **Number** | **Step** | **Expected Result** | **Result** |
>>> | 1 | Ensure that there is at least one other user who has access to the project | There is another user who can view the project | PASS |
>>> | 2 | As the project admin, navigate to the project storage page | Projects are listed | PASS |
>>> | 3 | Click on the three dotted button and select edit | A dialog is shown with the current storage information | PASS |
>>> | 4 | Add a second user to the storage and press save | Second user is added | PASS |
>>> | 5 | Log in to datalabs as the second user and navigate to the storage page| The project that user was added to is seen here | PASS |
>>> | 6 | Log back in as the administrator and remove the second user from the storage | Second user is removed | PASS |
>>> | 7 | Log back in as the second user and navigate to the storage page | The storage is no longer listed and the user can no longer access the storage. | PASS |

---

>>>###### **TC2.3.1.7 Storages are not shared between projects**
>>> | **JIRA ID(s):** | [NERCDL-788](https://jira.ceh.ac.uk/browse/NERCDL-788?jql=key%20in%20(NERCDL-788)) |OVERALL RESULT: | PASS |
>>> | ------------ | --------- | --------- | ------|
>>> | **Number** | **Step** | **Expected Result** | **Result** |
>>> | 1 | Ensure that the user has access to two projects each with storages | User has access to two projects each with storages | PASS |
>>> | 2 | Access the first project and view the notebooks section. Press the create a notebook button | Create a notebook dialog opens | PASS |
>>> | 3 | View the data store dropdown in the create a notebook dialog | Data stores for only the first project are shown | PASS |
>>> | 4 | Navigate to the notebooks section of the second project | Any pre-existing notebooks are present | PASS |
>>> | 5 | Click on the create notebooks button | The create notebooks dialog form is opened | PASS |
>>> | 6 | View the storage dropdown | Only data stores for the second project are shown here | PASS |
---
...
>>> | ------------ | --------- | --------- | ------|
>>> | **Number** | **Step** | **Expected Result** | **Result** |
>>> | 1 | Navigate to the Storage page of a project in which the logged in account has permission to create storage. | See list of any currently available data stores in the project with a Create Data Store button below it. | PASS |
>>> | 2 | Click the Create Data Store button. | Shown the create data store dialog with field for Storage Type | PASS |
>>> | 3 | Note the types that are available. | N/A | PASS |
>>> | 4 | Take a copy of the original storage config map then update it to change the available types and restart the web-app pod (this may require a developer). Reload the page and click the Create Data Store button. | Shown the create data store dialog where the values match those just set in the image config map. | PASS |
>>> | 5 | Reset the value of the config map to the original value and then restart the web-app pod (this may require a developer). | System is back to the same state as before the test was run. | PASS |
---
...
>>> | 1 | Navigate to the sites page | Pre-existing sites are listed here | PASS |
>>> | 2 | Click on the create button | A dialog is shown with the following fields: Display Name, Site Type, URL, Data Store, Source Path, Description and Visibility Status | PASS |
>>> | 3 | Fill out the required fields. Set the type to R studio | And extra field appears when R Shiny is selected. This is a version field. 4.0.1 is the default value | PASS |
>>> | 4 | Click on the version field | A dropdown with two options is displayed: 4.0.1 & 3.5.3 | PASS |
...
>>> | **JIRA ID(s):** | [NERCDL-783](https://jira.ceh.ac.uk/browse/NERCDL-783?jql=key%20in%20(NERCDL-783)) |OVERALL RESULT: | PASS |
>>> | ------------ | --------- | --------- | ------|
>>> | **Number** | **Step** | **Expected Result** | **Result** |
>>> | 1 | As a project admin, navigate to the project settings https://testlab.test-datalabs.nerc.ac.uk/projects/project-name-here/settings | User can access the project settings page for a project which they are an admin. A list of users with access to the project, and their permissions, are shown on this page. | PASS |
>>> | 2 | Click on the add user field | A drop-down of users are shown. The drop down is shown directly above or below the user field, depending on how far the page has been scrolled up or down | PASS |
>>> | 3 | Select a user from the the list | User is selected | PASS |
>>> | 4 | Select the permissions drop-down to the right of the user field | Three permission options are available: Admin, User or Viewer | PASS |
>>> | 5 | Select the user option | User option is selected | PASS |
>>> | 6 | Click on "Add" | User is added to the user permissions list below the add user field | PASS |
>>> | 7 | Sign into Datalabs as this user | User has access to the project | PASS |
---
...
>>> | **JIRA ID(s):** | [NERCDL-867](https://jira.ceh.ac.uk/browse/NERCDL-867?jql=key%20in%20(NERCDL-867)) |OVERALL RESULT: | PASS |
>>> | ------------ | --------- | --------- | ------|
>>> | **Number** | **Step** | **Expected Result** | **Result** |
>>> | 1 | Ensure that the oidc configmap is set to its alternate values (see the linked Jira issue above) | The oidc configmap is set to its alternate values | PASS |
>>> | 2 | Ensure that the user is logged out of Datalabs | User is logged out | PASS |
>>> | 3 | Navigate to https://testlab.test-datalabs.nerc.ac.uk/projects | User is re-directed to the sign in page. There is a sign up button | PASS |
>>> | 4 | Click on the sign up button | User is re-directed to https://testlab.test-datalabs.nerc.ac.uk/sign-up and an email is created. This email is addressed to appsupport@ceh.ac.uk | PASS |
>>> | 5 | Click on the home button | User is re-directed to the Datalabs homepage | PASS |
>>> | 6 | Log in to Datalabs | User is logged in and re-directed to https://testlab.test-datalabs.nerc.ac.uk/projects  | PASS |
---

>##### 2.6.2 Sign in
>>>###### **TC2.6.2.1 User can sign in**
>>> | **JIRA ID(s):** | [NERCDL-362](https://jira.ceh.ac.uk/browse/NERCDL-362?jql=key%20in%20(NERCDL-362)) |OVERALL RESULT: | PASS |
>>> | ------------ | --------- | --------- | ------|
>>> | **Number** | **Step** | **Expected Result** | **Result** |
>>> | 1 | Navigate to datalabs | User is shown the homepage that contains two buttons: Sign up and log in | PASS |
>>> | 2 | Click on Log in  | The log in dialog is shown and there are two fields for the user's email and password | PASS |
>>> | 3 | Enter an invalid email and password combination | "Wrong email or password" message is shown. User is not signed in | PASS |
>>> | 4 | Enter an email that does have access along with an incorrect password | "Wrong email or password" message is shown. User is not signed in | PASS |
>>> | 5 | Enter an email that does have access along with the correct password | User is logged in and shown the projects page | PASS |
>>> | 6 | View the web browser cookies for Data labs | There is an authorization cookie for test-datalabs.nerc.ac.uk | PASS |
---
...
>>> | ------------ | --------- | --------- | ------|
>>> | **Number** | **Step** | **Expected Result** | **Result** |
>>> | 1 | Ensure that the user is logged out of Datalabs | User is logged out | PASS |
>>> | 2 | Enter the following URL into the browser https://testlab.test-datalabs.nerc.ac.uk/projects | User cannot access the page and is prompted to log in | PASS |
>>> | 3 | Enter the following URL https://testlab.test-datalabs.nerc.ac.uk/projects/forraj/info | User cannot access the page and is prompted to log in | PASS |
>>> | 4 | Enter the following URL https://testlab.test-datalabs.nerc.ac.uk/projects/forraj/notebooks | User cannot access the page and is prompted to log in | PASS |
>>> | 5 | Enter the following URL https://testlab.test-datalabs.nerc.ac.uk/projects/forraj/storage | User cannot access the page and is prompted to log in  | PASS |
---
...
>>> | ------------ | --------- | --------- | ------|
>>> | **Number** | **Step** | **Expected Result** | **Result** |
>>> | 1 | Navigate to an incorrect Datalabs URL e.g. https://badurl.test-datalabs.nerc.ac.uk/ | User is taken to a Datalabs page that informs the user that the requested page could not be found. There is a return to homepage link | PASS |
>>> | 2 | Click on the return to homepage link | If the user is not already signed in, they are taken to the homepage page https://testlab.test-datalabs.nerc.ac.uk/. If the user is already signed in, they are re-directed to the projects overview page | PASS |
---
...
>>> | 2 | Click on the Admin button | User is directed to the admin page. A list of project's and their resources are shown here. If there are more than 5 projects then there are pagination controls | PASS |
...
>>>| 8 | Search for a project that does not exit | "No Options" are shown in the dropdown | PASS |
...
>>>| 11 | Tick the site checkbox | Only storages and sites are shown beneath each project | PASS |
...
>>> | 3 | Attempt to access https://testlab.test-datalabs.nerc.ac.uk/admin/users | User cannot access any user permissions| PASS |
---

>>>###### **TC2.7.3 User permissions api returns the correct permissions**
>>> | **JIRA ID(s):** | [NERCDL-786](https://jira.ceh.ac.uk/browse/NERCDL-786?jql=key%20in%20(NERCDL-786)) |OVERALL RESULT: | PASS |
>>> | ------------ | --------- | --------- | ------|
>>> | **Number** | **Step** | **Expected Result** | **Result** |
>>> | 1 | Log in as an admin user | User is logged in and is taken to the /projects page | PASS |
>>> | 2 | View the chrome developer tools, switch to the network tab and reload the page | A list of network calls are shown | PASS |
>>> | 3 | View the response for the second to last api call | Contains the projects the user has access to and the admin status:  "system:instance:admin". It also contains the catalogue status e.g. catalogue publisher | PASS |
>>> | 4 | Log out of Datalabs | User is logged out successfully | PASS |
>>> | 5 | Log in as a user that is not an admin | User is logged in and is taken to the /projects page | PASS |
>>> | 6 | Repeat steps 1-3 as the non-admin user | Api response only contains the projects the user has access to. It does not contain admin information | PASS |
---

>>>###### **TC2.7.4 Admin user has access to user management page**
>>> | **JIRA ID(s):** | [NERCDL-664](https://jira.ceh.ac.uk/browse/NERCDL-664?jql=key%20in%20(NERCDL-664)) |OVERALL RESULT: | PASS |
>>> | ------------ | --------- | --------- | ------|
>>> | **Number** | **Step** | **Expected Result** | **Result** |
>>> | 1 | Log into Datalabs as an admin user | User successfully logs in and there is an admin button in the top navigation menu | PASS |
>>> | 2 | Click on the admin button | User is taken to the resources admin page. There is a "Users" tab on the left side of the page | PASS |
>>> | 3 | Click on the "Users" tab | User is taken to the user management page | PASS |
---

>>>###### **TC2.7.4 User management page filters functionality tests**
>>> | **JIRA ID(s):** | [NERCDL-664](https://jira.ceh.ac.uk/browse/NERCDL-664?jql=key%20in%20(NERCDL-664)) |OVERALL RESULT: | PASS |
>>> | ------------ | --------- | --------- | ------|
>>> | **Number** | **Step** | **Expected Result** | **Result** |
>>> | 1 | Whilst on the user management page, click on the users field. **Note:** Ensure that the logged in account is a part of multiple projects and is the project admin for at least one project | A drop down of users are displayed. The current logged in user should be displayed here | PASS |
>>> | 2 | Start to type in a known username | The drop-down list is filtered | PASS |
>>> | 3 | Select a user by clicking on an item from the drop-down list | User is now displayed in the user field and only items relating to the user are shown on the page | PASS |
>>> | 4 | Enter a username that is not a user in the Datalabs system | "No options" message is shown in the drop-down | PASS |
>>> | 5 | Click on the X next to the selected user | User is removed from the field and the page is no longer filtered against the one user | PASS |
>>> | 6 | View the filter checkboxes | There are various admin, user and owner filters | PASS |
>>> | 7 | Select some filters e.g. instance admin and project admin | The page is filtered to only show users that meet at least one of these filter requirements. Checkboxes are clearly marked as selected | PASS |
>>> | 8 | Select only the project admin filter | The logged in user should be shown on the page along with any other users who are project admins. The number of projects is shown beneath the user's name on the page. | PASS |
>>> | 9 | Click on the user's name | A drop-down/page expansion shows all of the projects which the user has project admin status | PASS |
---

>>>###### **TC2.7.5 User management page filters functionality tests PART 2**
>>> | **JIRA ID(s):** | [NERCDL-789](https://jira.ceh.ac.uk/browse/NERCDL-789?jql=key%20in%20(NERCDL-789)) |OVERALL RESULT: | PASS |
>>> | ------------ | --------- | --------- | ------|
>>> | **Number** | **Step** | **Expected Result** | **Result** |
>>> | 1 | On the user management page, ensure that the following users have the following permissions: andrew.longmore@tessella.com - Catalogue admin, rajdeep.uppal@tessella.com - catalogue publisher, alex.lee@tessella.com - catalogue editor, dean.selby@tessella.com - catalogue user | Permissions are set | PASS |
>>> | 2 | Select the catalogue admin filter | Only users that are catalogue admins are shown e.g. andrew.longmore@tessella.com | PASS |
>>> | 3 | Select the catalogue publisher filter | Only users that are catalogue admins are shown e.g. rajdeep.uppal@tessella.com | PASS |
>>> | 4 | Select the catalogue editor editor filter | Only users that are catalogue editors are shown e.g. alex.lee@tessella.com | PASS |
>>> | 5 | Do not select any filters and expand dean.selby@tessella.com | User has catalogue publisher permissions set  | PASS |
---

>>>###### **TC2.7.6 Instance Admin tests**
>>> | **JIRA ID(s):** | [NERCDL-787](https://jira.ceh.ac.uk/browse/NERCDL-787?jql=key%20in%20(NERCDL-787)) |OVERALL RESULT: | PASS |
>>> | ------------ | --------- | --------- | ------|
>>> | **Number** | **Step** | **Expected Result** | **Result** |
>>> | 1 | As an instance admin, navigate to https://testlab.test-datalabs.nerc.ac.uk/admin/users | A list of users are shown along with their permissions | PASS |
>>> | 2 | Select the instance admin filter | All users that are instance admins are shown. The logged in user is also shown | PASS |
>>> | 3 | Attempt to remove the logged in user's instance admin status | User is unable to remove their own instance admin status | PASS |
>>> | 4 | Make a non-instance admin an instance admin | User is now an instance admin | PASS |
>>> | 5 | Log in as this user and attempt to access https://testlab.test-datalabs.nerc.ac.uk/admin/users | User can access this endpoint | PASS |
---

>>>###### **TC2.7.7 Catalogue service is controlled by config.yaml**
>>> | **JIRA ID(s):** | [NERCDL-790](https://jira.ceh.ac.uk/browse/NERCDL-790?jql=key%20in%20(NERCDL-790)) |OVERALL RESULT: | PASS |
>>> | ------------ | --------- | --------- | ------|
>>> | **Number** | **Step** | **Expected Result** | **Result** |
>>> | 1 | With the config set to show catalogue permissions, navigate to https://testlab.test-datalabs.nerc.ac.uk/admin/users | There are catalogue filters present on this page. These filters include admin, publisher and editor. There are also the following filters: instance admin, project user, project admin, project user, site owner, notebook owner and storage access | PASS |
>>> | 2 | Turn the catalogue permissions off in the config file. A developer may have to do this. | Permissions are turned off | PASS |
>>> | 3 | Navigate to https://testlab.test-datalabs.nerc.ac.uk/admin/users | The catalogue filters have been removed. The other filters listed in step 1 are still present. The layout of the filters and page has not been negatively effected by the removal of catalogue filters/permissions | PASS |
>>> | 4 | Select the filters currently present on the page | These filters still work and filter the users correctly | PASS |
---
...
>>> | ------------ | --------- | --------- | ------|
>>> | **Number** | **Step** | **Expected Result** | **Result** |
>>> | 1 | Navigate to '/assets/add-metadata' | Page and form load correctly | PASS |
>>> | 2 | Press the apply button without entering any text | All mandatory fields are highlighted in red and marked as mandatory | PASS |
>>> | 3 | Set the name field to a string e.g. Test1! | Field is set and there are no validation issues from using a string containing special characters | PASS |
>>> | 4 | Set the version field to a string e.g. Test1! | Field is set and there are no validation issues from using a string containing special characters | PASS |
>>> | 5 | Click on type | A drop-down is shown which contains three options: data-set, notebook,snippet | PASS |
>>> | 6 | Select a type from the drop-down | A type is selected an populates the field | PASS |
>>> | 7 | Set the file location to a path e.g. /Test1! | Field is set and there are no validation issues from using a path containing special characters | PASS |
>>> | 8 | Set the master URL to any valid URL e.g. https://google.com | URL is set. There are no validation errors. A new field appears beneath: Master Version | PASS |
>>> | 9 | Set the master version field to a string e.g Test1! | Field is set | PASS |
>>> | 10 | Click on the owners field | A drop-down of all users in datalabs is shown | PASS |
>>> | 11 | Begin to type an email that belongs to a user that has access to Datalabs | Drop-down is filtered to show the user that the email belongs to | PASS |
>>> | 12 | Select the user | user's email populates the owners field | PASS |
>>> | 13 | Click on visibility | A drop down is shown with two options: public and by project | PASS |
>>> | 14 | Click on by project | An extra field is shown beneath: projects | PASS |
>>> | 15 | Click on the projects field | A drop-down of all projects in Datalabs is shown | PASS |
>>> | 16 | Select a project | Selected project populates the field | PASS |
>>> | 17 | Click on apply | A dialog is shown asking the user to confirm or cancel | PASS |
>>> | 18 | Click on cancel | Form is not submitted. Changes to the form are preserved | PASS |
>>> | 19 | Click on apply | A dialog is shown asking the user to confirm or cancel | PASS |
>>> | 20 | Click on apply again | A confirmation in the UI is shown informing the user that the metadata is set. Asset id now has an actual id string. All fields are now read only. There is a button labelled "Add another" at the bottom | PASS |
---
...
>>> | ------------ | --------- | --------- | ------|
>>> | **Number** | **Step** | **Expected Result** | **Result** |
>>> | 1 | Ensure that a form has been submitted successfully | A form has been submitted successfully | PASS |
>>> | 2 | Attempt to submit an exact copy of a form that has already been submitted | Form is not submitted because the name & version are repeated, the file location is repeated and master URL & master version are repeated | PASS |
>>> | 3 | Attempt to submit a form where the name and version are the only fields that match the submitted form | Form is not submitted due to the name and version matching a submitted form | PASS |
>>> | 4 | Attempt to submit a form where the location is the only fields that match the submitted form | Form is not submitted due to the location matching a submitted form | PASS |
>>> | 5 | Attempt to submit a form where the master URL and master version are the only fields that match the submitted form | Form is not submitted due to the master name and master version matching a submitted form | PASS |
---
>#### 2.8.2 Find Asset
