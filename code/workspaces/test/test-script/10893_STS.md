...
###### TC[2.2.4.1.1] Create Conda Dask cluster

| **JIRA ID(s):** | [NERCDL-964](https://jira.ceh.ac.uk/browse/NERCDL-964) |OVERALL RESULT: | PASS |
| ------------ | --------- | --------- | ------|
| **Number** | **Step** | **Expected Result** | **Result** |
...
| 5 | Fill out the cluster info, including the volume mount that the notebook is on and setting the Conda path to `/data/conda/myConda`.  Choose minimum values for other fields, to reduce the load on the test cluster | Fields can be completed | Pass |
| 5 | Click Create | The dialog closes, and the new Dask cluster appears in the list | Pass |
| 5 | Make a note of the scheduler address for the next step.  Start a new notebook in your lab, using the myConda environment. | The notebook opens | Pass |
| 5 | In a cell, put the below and run the cell. | A message tells you there is one worker.  There are no errors are warnings. | Pass |
